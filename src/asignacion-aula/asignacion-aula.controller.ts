...
export class AsignacionAulaController {
  ...
  create(@Body() createAsignacionAulaDto: CreateAsignacionAulaDto) {
    return this.asignacionAulaService.create(createAsignacionAulaDto);
  }

  @Get()
  @ApiOperation({ summary: 'Obtener todas las asignaciones de aulas a grados' })
  findAll() {
    return this.asignacionAulaService.findAll();
  }

  @Get(':id')
  @ApiOperation({ summary: 'Obtener una asignación de aula específica por ID' })
  findOne(@Param('id') id: string) {
    return this.asignacionAulaService.findOne(id);
  }

  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar una asignación de aula' })
  update(@Param('id') id: string, @Body() updateAsignacionAulaDto: UpdateAsignacionAulaDto) {
    return this.asignacionAulaService.update(id, updateAsignacionAulaDto);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Eliminar una asignación de aula (desactivar)' })
  remove(@Param('id') id: string) {
    return this.asignacionAulaService.remove(id);
  }
}
