...
export class UsuarioService {
  ...
  async create(createUsuarioDto: CreateUsuarioDto): Promise<{ sucess: boolean; message: string; usuario: Usuario }> {
    const saltRounds = 10;
    const hashedPassword = await bcrypt.hash(createUsuarioDto.contrasena, saltRounds);
  ...
  async findAll(): Promise<{ sucess: boolean; message: string; usuarios: Usuario[] }> {
  ...
  async update(id: string, updateUsuarioDto: UpdateUsuarioDto): Promise<{ sucess: boolean; message: string; usuario: Usuario }> {
    ...
    if (updateUsuarioDto.contrasena) {
      const saltRounds = 10;
      updateData.contrasena = await bcrypt.hash(updateUsuarioDto.contrasena, saltRounds);
    }
  ...
  async changePassword(userId: string, changePasswordDto: ChangePasswordDto): Promise<{ success: boolean; message: string }> {
    const { contrasenaActual, nuevaContrasena, confirmarContrasena } = changePasswordDto;
